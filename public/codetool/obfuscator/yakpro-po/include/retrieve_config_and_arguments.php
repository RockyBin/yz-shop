...
if ($config_filename==''){
	if(php_sapi_name() == 'cli') fprintf(STDERR,"Warning:No config file found... using default values!%s",PHP_EOL);
}
else
{
            ...
            if (is_file($source_file) && is_readable($source_file))
            {
                ...
                if ( ($target_file!=='') && file_exists($target_file) )
                {
                    ...
                    if ( is_readable($x) && is_writable($x) && is_file($x) )
                    {
                        ...
                        if (strpos($y,'    |  Obfuscated by Php Obfuscator ')===false)       // comment is a magic string, used to not overwrite wrong files!!!
                        {
                            $x = realpath($target_file);
