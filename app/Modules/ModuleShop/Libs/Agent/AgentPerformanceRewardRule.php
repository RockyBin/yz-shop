 ...
 * 团队代理推荐奖设置
 */
class AgentPerformanceRewardRule
{
    private $_model = null;
    ...
    public function add(array $param, $reload = false)
    {
        if ($param) {
            ...
            $model = new AgentPerformanceRewardRuleModel();
            $model->fill($param);
            $model->save();
            if ($reload) {
    ...
    {
        if ($id) {
            $model = AgentPerformanceRewardRuleModel::query()
    ...
    {
        $query = AgentPerformanceRewardRuleModel::query()
            ->where('site_id', Site::getCurrentSite()->getSiteId());
        // 搜索条件
        ...
        $total = $query->count();
        // 排序
        if ($param['order_by'] && Schema::hasColumn('tbl_agent_performance_reward_rule', $param['order_by'])) {
    ...
    {
        // 代理等级
        if (is_numeric($param['agent_level'])) {
            $query->where('agent_level', intval($param['agent_level']));
        }
        // 奖励类型
