...
class SupplierSettleAdmin
{
    ...
    public function getList($param)
    {
        ...
        if ($total > 0) {
            ...
            $itemQuery = OrderItemModel::query()
                ...
                ->leftJoin('tbl_after_sale_item as saleafter', 'item.id', 'saleafter.order_item_id')
                ->whereIn('item.order_id', $orderIds)
                ->addSelect(DB::raw('distinct(item.id)'))
    ...
    {
        $productData->all_discount = $productData->total_discount; // 优惠
    }
    ...
    private function setQuery(Builder $query, $param)
    {
        $query->from('tbl_order as order')
            ...
            ->where('order.site_id', $this->siteId)->where('order.supplier_member_id', '>', 0)->where('settle.id','>',0);
        // 结算状态
