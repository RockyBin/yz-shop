...
class Connection implements ConnectionInterface
{
    ...
    public function affectingStatement($query, $bindings = [])
    {
        return $this->run($query, $bindings, function ($query, $bindings) {
            ...
            // 数据表的全局更新事件通知，用于刷新缓存
            $onDbUpdated = config('ON_DB_UPDATED');
            if (!$onDbUpdated) $onDbUpdated = env('ON_DB_UPDATED');
            if ($onDbUpdated) {
                $classInfo = explode('@',$onDbUpdated);
                call_user_func_array(array(new $classInfo[0](), $classInfo[1] ? $classInfo[1] : "handle"), [$query, $bindings]);
            }
